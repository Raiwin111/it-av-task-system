...
// Location options come from the configurable task_locations table so teams can
// adapt the system without editing code. Falls back to generic demo rooms when
// the table is empty or unavailable.
function task_location_options(mysqli $conn): array
{
    static $cached_options = null;
    if ($cached_options !== null) return $cached_options;

    $fallback = ["ห้องประชุม A", "ห้องประชุม B", "ล็อบบี้", "ห้องจัดเลี้ยง 1"];
    try {
        $result = $conn->query("SELECT name FROM task_locations WHERE is_enabled = 1 ORDER BY sort_order, name");
        $options = [];
        if ($result) {
            while ($row = $result->fetch_assoc()) $options[] = (string) $row["name"];
        }
        $cached_options = $options ?: $fallback;
    } catch (Throwable) {
        $cached_options = $fallback;
    }
