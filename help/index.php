        ...
        <section class="card form-card mb-4">
            ...
            <div class="card-body p-4">
                <div class="row g-3">
                    ...
                    <div class="col-md-6 col-xl-3"><a class="help-route" href="../report/"><span class="route-icon text-warning"><i class="bi bi-card-list"></i></span><span><strong>รายการงาน</strong><small>ค้นหา ดูรายละเอียด และแก้ไขงาน</small></span></a></div>
                    <div class="col-md-6 col-xl-3"><a class="help-route" href="../config/"><span class="route-icon text-info"><i class="bi <?php echo strtoupper((string) ($_SESSION["role"] ?? "USER")) === "ADMIN" ? "bi-gear" : "bi-key"; ?>"></i></span><span><strong><?php echo strtoupper((string) ($_SESSION["role"] ?? "USER")) === "ADMIN" ? "ตั้งค่าระบบ" : "ตั้งค่าบัญชี"; ?></strong><small><?php echo strtoupper((string) ($_SESSION["role"] ?? "USER")) === "ADMIN" ? "System Settings และจัดการผู้ใช้" : "เปลี่ยนรหัสผ่านของคุณ"; ?></small></span></a></div>
                </div>
            </div>
        </section>
        ...
        <section class="card form-card">
            ...
            <div class="card-body p-3 p-lg-4">
                <div class="accordion help-accordion" id="helpGuideAccordion">
                    ...
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="guideReportHeading"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#guideReport"><span class="guide-number">3</span><span><strong>รายการงาน</strong><small>Task List สำหรับค้นหาและติดตามงาน</small></span></button></h2>
                    ...
                    <div class="accordion-item">
                        <h2 class="accordion-header" id="guideConfigHeading"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#guideConfig"><span class="guide-number">4</span><span><strong>ตั้งค่าบัญชีและระบบ</strong><small>เปลี่ยนรหัสผ่าน และ User Management สำหรับ ADMIN</small></span></button></h2>
                        <div id="guideConfig" class="accordion-collapse collapse" data-bs-parent="#helpGuideAccordion"><div class="accordion-body"><p>ทุก Role เข้า Config เพื่อเปลี่ยนรหัสผ่านของตนเองได้ ส่วน User Management สงวนไว้สำหรับ ADMIN</p><ul><li>ระบบไม่มีหน้าสมัครสมาชิกสาธารณะ บัญชีใหม่ต้องสร้างโดย ADMIN จาก Config</li><li>USER และ SUPER เห็นเฉพาะช่องรหัสผ่านเดิม รหัสผ่านใหม่ และยืนยันรหัสผ่านใหม่</li><li>ADMIN สร้างและแก้ไขบัญชี กำหนดทีม/บทบาท รีเซ็ตรหัสผ่าน และเปิดหรือปิดบัญชีได้</li><li>บัญชีที่มีประวัติงานไม่สามารถลบได้ เพื่อรักษาความถูกต้องของข้อมูลรายการงาน</li><li>จำนวนผู้ใช้งานล่าสุดนับจากบัญชีที่มีการใช้งานภายใน 5 นาที</li></ul></div></div>
                    </div>
                    <div class="accordion-item">
...
<script>
    ...
    const detailedGuide = {
        ...
        guideConfig: `
            <div class="guide-detail"><h3>วิธีใช้หน้าตั้งค่าบัญชีและระบบอย่างละเอียด</h3><h4>1. เปลี่ยนรหัสผ่านของตนเอง</h4><ol><li>USER, SUPER และ ADMIN เข้า Config ได้ทุก Role</li><li>กรอกรหัสผ่านเดิม รหัสผ่านใหม่อย่างน้อย 8 ตัวอักษร และยืนยันรหัสผ่านใหม่</li><li>หลังเปลี่ยนรหัสผ่าน Remember Me เดิมจะถูกยกเลิกเพื่อความปลอดภัย</li></ol><h4>2. การเพิ่มผู้ใช้งาน</h4><ol><li>ระบบไม่มีหน้า Create Account หรือระบบรออนุมัติ</li><li>ADMIN สร้างบัญชีจาก Config และกำหนดชื่อผู้ใช้ ชื่อ-นามสกุล รหัสผ่านเริ่มต้น ทีม บทบาท และสถานะ</li><li>บัญชีที่ปิดใช้งานจะไม่สามารถเข้าสู่ระบบหรือใช้ Session เดิมต่อได้</li></ol><h4>3. สิทธิ์ของ ADMIN</h4><ol><li>แก้ไขชื่อ ทีม บทบาท และสถานะบัญชี</li><li>รีเซ็ตรหัสผ่านและเปิดหรือปิดบัญชีผู้ใช้งาน</li><li>ลบผู้ใช้งานได้เฉพาะบัญชีที่ยังไม่มีประวัติงาน และไม่สามารถลบบัญชีของตนเองได้</li><li>ตรวจสอบเวอร์ชัน จำนวนผู้ใช้ล่าสุด และจำนวนงานในส่วนข้อมูลระบบ</li></ol><div class="guide-note">SUPER จัดการงานข้ามทีมได้ แต่ User Management สงวนไว้สำหรับ ADMIN เท่านั้น</div></div>`,
        guideHelp: `
