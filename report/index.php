...
if (($_SERVER["REQUEST_METHOD"] ?? "GET") === "POST" && ($_POST["action"] ?? "") === "delete") {
    ...
    $owner_stmt = $conn->prepare("SELECT created_by FROM tasks WHERE id = ? AND is_deleted = 0 LIMIT 1");
    $owner_stmt->bind_param("i", $delete_id);
    $owner_stmt->execute();
    $delete_task = $owner_stmt->get_result()->fetch_assoc();
    $owner_stmt->close();

    $can_delete = $delete_task && ($role !== "USER" || (int) $delete_task["created_by"] === $user_id);
...
$sql = "SELECT t.*, COALESCE(u.username, 'Unknown user') AS created_by_name FROM tasks t LEFT JOIN users u ON u.id = t.created_by WHERE t.is_deleted = 0 ORDER BY t.created_at DESC, t.id DESC";
...
function can_edit_task(array $task, string $role, int $user_id): bool { return $role !== "USER" || (int) $task["created_by"] === $user_id; }
function can_delete_task(array $task, string $role, int $user_id): bool { return $role !== "USER" || (int) $task["created_by"] === $user_id; }
...
<style>
    ...
    .report-page > section.row.g-4 > div:nth-child(1) .form-card { background: #eaf4ff; border-color: #d5e7f9; box-shadow: 0 8px 20px rgba(38, 93, 145, .10); }
    .report-page > section.row.g-4 > div:nth-child(2) .form-card { background: #fff4e5; border-color: #f1ddc1; box-shadow: 0 8px 20px rgba(134, 93, 35, .09); }
    .report-page > section.row.g-4 > div:nth-child(3) .form-card { background: #ecfff1; border-color: #d5efdd; box-shadow: 0 8px 20px rgba(41, 111, 70, .09); }
    .report-page > section.row.g-4 > div:nth-child(4) .form-card { background: #ffecec; border-color: #f2d5d5; box-shadow: 0 8px 20px rgba(135, 51, 58, .08); }    /* Align Report components with the shared Dashboard and Task Input design language. */
...
</script><?php require_once __DIR__ . "/../includes/app_footer.php"; ?>
