                                ...
                                <div class="row g-4">
                                    ...
                                    <div class="col-12" id="avEquipmentGuide">
                                        <div class="task-equipment-guide">
                                            ...
                                            <div class="mt-3" id="equipmentRows">
                                                <?php foreach ($equipment_items as $equipment_item): ?>
                                                    <?php $equipment_id = (int) $equipment_item["id"]; $equipment_selected = isset($posted_equipment_rows[$equipment_id]); ?>
                                                    <div class="task-equipment-row" data-equipment-row>
                                                        <div class="form-check mb-0">
                                                            <input class="form-check-input" type="checkbox" name="equipment_id[]" value="<?php echo $equipment_id; ?>" id="equipment<?php echo $equipment_id; ?>"<?php echo $equipment_selected ? " checked" : ""; ?>>
                                                            <label class="form-check-label fw-semibold" for="equipment<?php echo $equipment_id; ?>"><?php echo htmlspecialchars($equipment_item["name"], ENT_QUOTES, "UTF-8"); ?></label>
                                                        </div>
                                                        <div class="task-equipment-quantity">
                                                            <button class="btn btn-outline-secondary" type="button" data-quantity-action="decrease" aria-label="ลดจำนวน <?php echo htmlspecialchars($equipment_item["name"], ENT_QUOTES, "UTF-8"); ?>">−</button>
                                                            <input class="form-control text-center" type="number" name="equipment_quantity[<?php echo $equipment_id; ?>]" value="<?php echo $equipment_selected ? (int) $posted_equipment_rows[$equipment_id] : 1; ?>" min="1" step="1" aria-label="จำนวน <?php echo htmlspecialchars($equipment_item["name"], ENT_QUOTES, "UTF-8"); ?>">
                                                            <button class="btn btn-outline-secondary" type="button" data-quantity-action="increase" aria-label="เพิ่มจำนวน <?php echo htmlspecialchars($equipment_item["name"], ENT_QUOTES, "UTF-8"); ?>">+</button>
                                                        </div>
                                                    </div>
                                                <?php endforeach; ?>
                                            </div>
...
<script>
    ...
    const updateEquipmentRowState = (row, isAV = departmentControl?.value === "AV") => {
        const checkbox = row.querySelector('input[type="checkbox"]');
        const quantity = row.querySelector('input[type="number"]');
        const quantityButtons = row.querySelectorAll("button");
        if (!checkbox || !quantity) return;
        checkbox.disabled = !isAV;
        quantity.disabled = !isAV || !checkbox.checked;
        quantityButtons.forEach((button) => { button.disabled = !isAV || !checkbox.checked; });
    };

    equipmentRows?.querySelectorAll("[data-equipment-row]").forEach((row) => {
        const checkbox = row.querySelector('input[type="checkbox"]');
        const quantity = row.querySelector('input[type="number"]');
        checkbox?.addEventListener("change", () => updateEquipmentRowState(row));
    ...
    const updateITWorkflow = () => {
        ...
        equipmentRows?.querySelectorAll("[data-equipment-row]").forEach((row) => updateEquipmentRowState(row, isAV));
