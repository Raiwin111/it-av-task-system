...
$common_problem_result = $conn->query("SELECT category, COUNT(*) AS total FROM tasks WHERE is_deleted = 0 AND TRIM(COALESCE(category, '')) <> '-' GROUP BY category ORDER BY total DESC, category ASC LIMIT 5");
$common_problem_rows = [];
while ($row = $common_problem_result->fetch_assoc()) {
    $common_problem_rows[] = $row;
}
$common_problem_text_result = $conn->query("SELECT problem, COUNT(*) AS total FROM tasks WHERE is_deleted = 0 AND TRIM(COALESCE(problem, '')) NOT IN ('', '-') GROUP BY problem ORDER BY total DESC, problem ASC LIMIT 5");
$common_problem_text_rows = [];
while ($row = $common_problem_text_result->fetch_assoc()) {
    $common_problem_text_rows[] = $row;
}
$common_problem_max = max(1, (int) ($common_problem_rows[0]["total"] ?? 0));
