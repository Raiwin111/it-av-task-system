...
if (($_SERVER["REQUEST_METHOD"] ?? "GET") === "POST" && report_post_string("action") === "update") {
    ...
    if (!hash_equals($report_task_csrf, report_post_string("csrf_token"))) {
        ...
        if ($update_stmt->execute()) {
            ...
            if ($equipment_saved && $activity_saved && $problem_option_saved) {
                $conn->commit();
...
$task_equipment = is_array($task["equipment"] ?? null) ? $task["equipment"] : [];
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <?php if ($task_has_work_description || $task_has_work_action || $task_has_remark || $task_equipment): ?>
                    <section class="task-detail-section" aria-labelledby="taskNarrativeHeading<?php echo $task["id"]; ?>">
                        ...
                        <div class="task-detail-narrative">
                            ...
                            <?php if (!$task_is_it && $task_equipment): ?><div class="task-detail-field" data-detail-field="equipment"><strong class="task-detail-label">อุปกรณ์ที่ใช้งาน</strong><div class="d-flex flex-wrap gap-2 mt-2"><?php foreach ($task_equipment as $equipment_item): ?><span class="badge rounded-pill text-bg-light border text-dark fw-normal"><?php echo htmlspecialchars($equipment_item["name"], ENT_QUOTES, "UTF-8"); ?> × <?php echo (int) $equipment_item["quantity"]; ?></span><?php endforeach; ?></div></div><?php endif; ?>
