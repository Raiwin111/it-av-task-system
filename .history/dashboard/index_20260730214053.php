...
$problem_stat_result = $conn->query("SELECT category, COUNT(*) AS total FROM tasks WHERE is_deleted = 0 AND TRIM(COALESCE(category, '')) <> '-' GROUP BY category ORDER BY total DESC, category ASC LIMIT 5");
$problem_stats = [];
while ($row = $problem_stat_result->fetch_assoc()) {
    $problem_stats[] = $row;
}
$problem_text_result = $conn->query("SELECT problem, COUNT(*) AS total FROM tasks WHERE is_deleted = 0 AND TRIM(COALESCE(problem, '')) NOT IN ('', '-') GROUP BY problem ORDER BY total DESC, problem ASC LIMIT 5");
$problem_text_stats = [];
while ($row = $problem_text_result->fetch_assoc()) {
    $problem_text_stats[] = $row;
}
$max_problem_stat = max(1, (int) ($problem_stats[0]["total"] ?? 0));
