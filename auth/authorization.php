...
{
    if (!isset($task["department"])) return false;
    // ADMIN manages every team. SUPER sees all teams but edits only its own;
    // USER is limited to the assigned team as well.
    if (current_role() === "ADMIN") return true;
    return strtoupper(trim((string) $task["department"])) === strtoupper(trim(current_department()));
}
