...
if (!$is_admin) {
    header("Location: ../account_settings/?error=config_forbidden");
    exit;
}
...
function config_password_meets_policy(string $password): bool
{
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (in_array($action, ["update_own_profile", "change_own_password"], true)) {
        http_response_code(400);
        $form_error = "การตั้งค่าบัญชีถูกย้ายไปหน้า Account Settings แล้ว";
        ...
        if ($action === "update_own_profile") {
                ...
                if ($username_used) {
                    ...
                    if (isset($_FILES["profile_image"]) && $_FILES["profile_image"]["error"] !== UPLOAD_ERR_NO_FILE) {
                        ...
                        if ($image["error"] !== UPLOAD_ERR_OK || $image["size"] > 2 * 1024 * 1024 || !isset($allowed_mimes[$mime])) {
                            $form_error = "รูปโปรไฟล์ต้องเป็น JPG, PNG หรือ WebP ขนาดไม่เกิน 2 MB";
                        } else {
                    ...
                    if ($form_error === "") {
                        $profile_save_stmt = $conn->prepare("UPDATE users SET username = ?, full_name = NULLIF(?, ''), profile_image = ? WHERE id = ?");
                        $profile_save_stmt->bind_param("sssi", $username, $full_name, $stored_image, $current_user_id);
                        $profile_saved = $profile_save_stmt->execute();
                        $profile_save_stmt->close();
                        if ($profile_saved) {
                            ...
                            config_redirect("profile_updated");
                        }
            ...
            } elseif (!config_password_meets_policy($new_password)) {
            ...
            if ($user_id <= 0 || $username === "" || strlen($username) > 50 || strlen($full_name) > 120 || !in_array($department, $departments, true) || !in_array($role, $roles, true) || ($new_password !== "" && strlen($new_password) < 8)) {
                ...
                if ($username_exists) {
                    ...
                    $password_hash = password_hash($new_password, PASSWORD_DEFAULT);
                    $edit_stmt = $conn->prepare("UPDATE users SET username = ?, full_name = NULLIF(?, ''), password = ?, department = ?, role = ?, is_enabled = ?, is_approved = 1, failed_login_attempts = 0, lock_until = NULL WHERE id = ?");
        ...
        if ($action === "reset_password") {
            ...
            if ($user_id <= 0 || strlen($new_password) < 8 || !hash_equals($new_password, $confirm_password)) {
                ...
                $password_hash = password_hash($new_password, PASSWORD_DEFAULT);
                $password_stmt = $conn->prepare("UPDATE users SET password = ?, failed_login_attempts = 0, lock_until = NULL WHERE id = ?");
        ...
        if ($action === "add_location") {
            ...
            if ($location_name === "" || mb_strlen($location_name) > 100) {
                $form_error = "กรุณาระบุชื่อสถานที่ไม่เกิน 100 ตัวอักษร";
            } else {
                $dup_stmt = $conn->prepare("SELECT id FROM task_locations WHERE name = ? LIMIT 1");
                $dup_stmt->bind_param("s", $location_name);
                $dup_stmt->execute();
                $duplicate = (bool) $dup_stmt->get_result()->fetch_assoc();
                $dup_stmt->close();

                if ($duplicate) {
                    $form_error = "มีสถานที่นี้อยู่ในระบบแล้ว";
                } else {
                    $order_result = $conn->query("SELECT COALESCE(MAX(sort_order), 0) + 1 AS next_order FROM task_locations");
                    $next_order = (int) ($order_result->fetch_assoc()["next_order"] ?? 1);
                    $insert_stmt = $conn->prepare("INSERT INTO task_locations (name, is_enabled, sort_order) VALUES (?, 1, ?)");
                    $insert_stmt->bind_param("si", $location_name, $next_order);
                    $insert_stmt->execute();
                    $insert_stmt->close();
                    config_redirect("location_added");
                }
            }
        }
        ...
        if ($action === "toggle_location") {
            ...
            if ($location_id <= 0) {
                $form_error = "ไม่พบสถานที่ที่ต้องการแก้ไข";
            } else {
                $toggle_stmt = $conn->prepare("UPDATE task_locations SET is_enabled = 1 - is_enabled WHERE id = ?");
                $toggle_stmt->bind_param("i", $location_id);
                $toggle_stmt->execute();
                $toggle_stmt->close();
                config_redirect("location_updated");
            }
        }
        ...
        if ($action === "delete_location") {
            $location_id = (int) ($_POST["location_id"] ?? 0);
            if ($location_id <= 0) {
                $form_error = "ไม่พบสถานที่ที่ต้องการลบ";
            } else {
                // Existing tasks keep their stored location text; only the option is removed.
                $delete_location_stmt = $conn->prepare("DELETE FROM task_locations WHERE id = ?");
                $delete_location_stmt->bind_param("i", $location_id);
                $delete_location_stmt->execute();
                $delete_location_stmt->close();
                config_redirect("location_deleted");
            }
        }
    }
}
...
if ($is_admin) {
    ...
    $locations = [];
    $location_result = $conn->query("SELECT id, name, is_enabled, sort_order, created_at FROM task_locations ORDER BY sort_order, id");
    if ($location_result) {
        while ($location = $location_result->fetch_assoc()) $locations[] = $location;
    }
}

$app_page_title = "System Config | IT / AV Task Management System";
$app_stylesheets = ["config.css?v=" . (string) filemtime(__DIR__ . "/config.css")];
$active_nav = "config";
...
<div class="app-shell d-flex">
    ...
    <main class="main-content flex-grow-1 p-4 p-lg-5">
        <div class="mb-4"><h1 class="page-heading h3 fw-bold mb-1">System Config</h1><p class="page-subtitle mb-0">จัดการผู้ใช้งานและตรวจสอบข้อมูลระบบสำหรับผู้ดูแลระบบ</p></div>

        <?php if (isset($_GET["saved"])): ?><div class="alert alert-success">บันทึกการตั้งค่าระบบเรียบร้อยแล้ว</div><?php endif; ?>
        ...
        <section class="card form-card mb-4">
            <div class="card-header d-flex align-items-center justify-content-between gap-3">
                <div class="d-flex align-items-center gap-2"><span class="section-icon d-inline-flex align-items-center justify-content-center"><i class="bi bi-geo-alt"></i></span><div><h2 class="section-title mb-0">สถานที่ปฏิบัติงาน</h2><p class="text-muted small mb-0">รายการสถานที่ที่แสดงในฟอร์มบันทึกงาน — ปรับแต่งได้โดยไม่ต้องแก้โค้ด</p></div></div>
                <form class="d-flex align-items-center gap-2" method="post">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION["config_csrf_token"], ENT_QUOTES, "UTF-8"); ?>">
                    <input type="hidden" name="action" value="add_location">
                    <input type="text" class="form-control form-control-sm" name="location_name" maxlength="100" placeholder="ชื่อสถานที่ใหม่" required style="min-width:220px;">
                    <button class="btn btn-primary btn-sm" type="submit"><i class="bi bi-plus-lg me-1"></i>เพิ่มสถานที่</button>
                </form>
            </div>
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0"><thead><tr><th class="ps-4 py-3">ชื่อสถานที่</th><th>ลำดับ</th><th>สถานะ</th><th>วันที่เพิ่ม</th><th class="pe-4 text-end">การจัดการ</th></tr></thead><tbody>
                    <?php if (!$locations): ?><tr><td colspan="5" class="text-center text-muted py-4">ยังไม่มีสถานที่ในระบบ — เพิ่มรายการแรกด้านบน</td></tr><?php endif; ?>
                    <?php foreach ($locations as $location): ?>
                        <tr><td class="ps-4 fw-semibold"><?php echo htmlspecialchars($location["name"], ENT_QUOTES, "UTF-8"); ?></td><td><?php echo (int) $location["sort_order"]; ?></td><td><?php if ((int) $location["is_enabled"] === 1): ?><span class="badge text-bg-success">แสดงในฟอร์ม</span><?php else: ?><span class="badge text-bg-secondary">ซ่อนอยู่</span><?php endif; ?></td><td><?php echo config_thai_date($location["created_at"]); ?></td><td class="pe-4 text-end"><form class="d-inline" method="post"><input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION["config_csrf_token"], ENT_QUOTES, "UTF-8"); ?>"><input type="hidden" name="action" value="toggle_location"><input type="hidden" name="location_id" value="<?php echo (int) $location["id"]; ?>"><button class="btn btn-sm <?php echo (int) $location["is_enabled"] === 1 ? "btn-outline-secondary" : "btn-outline-success"; ?>" type="submit"><?php echo (int) $location["is_enabled"] === 1 ? "ซ่อน" : "แสดง"; ?></button></form> <form class="d-inline" method="post" data-location-delete><input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION["config_csrf_token"], ENT_QUOTES, "UTF-8"); ?>"><input type="hidden" name="action" value="delete_location"><input type="hidden" name="location_id" value="<?php echo (int) $location["id"]; ?>"><button class="btn btn-sm btn-outline-danger" type="submit">ลบ</button></form></td></tr>
                    <?php endforeach; ?>
                </tbody></table>
            </div>
        </section>
...
<?php endif; ?>
<script>
    // Location deletion keeps existing task records intact, but still ask first.
    document.querySelectorAll("[data-location-delete]").forEach((form) => form.addEventListener("submit", (event) => {
        if (!window.confirm("ยืนยันการลบสถานที่นี้ออกจากรายการ? (งานเดิมจะยังเก็บข้อความสถานที่เดิมไว้)")) event.preventDefault();
    }));
</script>
