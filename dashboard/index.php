...
// All users can view the five latest tasks, regardless of creator.
$recent_tasks = $conn->query("SELECT t.*, u.username AS created_by_name FROM tasks t INNER JOIN users u ON u.id = t.created_by WHERE t.is_deleted = 0 ORDER BY t.created_at DESC, t.id DESC LIMIT 5");
$recent_task_rows = $recent_tasks->fetch_all(MYSQLI_ASSOC);
$recent_tasks->data_seek(0);
// Real chart data: all queries exclude soft-deleted tasks.
$monthly_labels = [];
$monthly_values = [];
$monthly_result = $conn->query("SELECT DATE_FORMAT(created_at, '%Y-%m') AS month_key, COUNT(*) AS total FROM tasks WHERE is_deleted = 0 GROUP BY month_key ORDER BY month_key ASC");
while ($row = $monthly_result->fetch_assoc()) {
    $monthly_labels[] = $row["month_key"];
    $monthly_values[] = (int) $row["total"];
}

$department_counts = ["IT" => 0, "AV" => 0];
$department_result = $conn->query("SELECT department, COUNT(*) AS total FROM tasks WHERE is_deleted = 0 GROUP BY department");
while ($row = $department_result->fetch_assoc()) {
    if (isset($department_counts[$row["department"]])) $department_counts[$row["department"]] = (int) $row["total"];
}

$category_labels = [];
$category_values = [];
$category_result = $conn->query("SELECT category, COUNT(*) AS total FROM tasks WHERE is_deleted = 0 GROUP BY category ORDER BY total DESC, category ASC LIMIT 5");
while ($row = $category_result->fetch_assoc()) {
    $category_labels[] = $problem_category_options[$row["category"]] ?? $row["category"];
...
<section class="mb-5" aria-labelledby="dashboard-charts-heading">
    <div class="mb-3"><h2 id="dashboard-charts-heading" class="page-heading h4 fw-bold mb-1">ภาพรวมกิจกรรม</h2><p class="text-muted mb-0">สถิติงานจากฐานข้อมูลปัจจุบัน</p></div>
    <div class="row g-4 mb-4">
        <div class="col-lg-6"><article class="card monitor-widget h-100"><div class="card-header py-3 px-4"><h3 class="page-heading h5 fw-bold mb-0">จำนวนงานรายเดือน</h3></div><div class="card-body"><div class="dashboard-chart"><canvas id="monthlyTasksChart"></canvas></div></div></article></div>
        <div class="col-lg-6"><article class="card monitor-widget h-100"><div class="card-header py-3 px-4"><h3 class="page-heading h5 fw-bold mb-0">สถิติแผนก IT และ AV</h3></div><div class="card-body"><div class="dashboard-chart"><canvas id="departmentTasksChart"></canvas></div></div></article></div>
    </div>
    <div class="row g-4 mb-4"><div class="col-12"><article class="card monitor-widget"><div class="card-header py-3 px-4"><h3 class="page-heading h5 fw-bold mb-0">ประเภทปัญหาที่พบบ่อย</h3></div><div class="card-body"><div class="dashboard-chart dashboard-chart-wide"><canvas id="topCategoriesChart"></canvas></div></div></article></div></div>
    <details class="card monitor-widget additional-information"><summary class="card-header d-flex align-items-center py-3 px-4"><span class="section-icon d-inline-flex align-items-center justify-content-center me-2"><i class="bi bi-plus-circle"></i></span><span class="page-heading h5 fw-bold">ส่วนที่เพิ่มเติมในอนาคต</span></summary><div class="card-body text-muted"></div></details>
...
<script>
    ...
    const dashboardMonthlyLabels = <?php echo json_encode($monthly_labels, JSON_UNESCAPED_UNICODE); ?>;
    const dashboardMonthlyValues = <?php echo json_encode($monthly_values); ?>;
    const dashboardDepartmentLabels = <?php echo json_encode(array_keys($department_counts), JSON_UNESCAPED_UNICODE); ?>;
    const dashboardDepartmentValues = <?php echo json_encode(array_values($department_counts)); ?>;
    const dashboardCategoryLabels = <?php echo json_encode($category_labels, JSON_UNESCAPED_UNICODE); ?>;
    const dashboardCategoryValues = <?php echo json_encode($category_values); ?>;
    ...
    if (window.Chart) {
        ...
        new Chart(document.getElementById("monthlyTasksChart"), {
            type: "line",
            data: { labels: dashboardMonthlyLabels, datasets: [{ label: "จำนวนงาน", data: dashboardMonthlyValues, borderColor: "#1769c2", backgroundColor: "rgba(23,105,194,.12)", fill: true, tension: .35, borderWidth: 3, pointRadius: 4 }] },
            options: { ...commonOptions, scales: { y: { beginAtZero: true, ticks: { precision: 0 } } } }
        });

        new Chart(document.getElementById("departmentTasksChart"), {
            type: "doughnut",
            data: { labels: dashboardDepartmentLabels, datasets: [{ data: dashboardDepartmentValues, backgroundColor: ["#1769c2", "#3f8f73"], borderWidth: 2, borderColor: "#fff" }] },
            options: commonOptions
        });

        new Chart(document.getElementById("topCategoriesChart"), {
            type: "bar",
            data: { labels: dashboardCategoryLabels, datasets: [{ label: "จำนวนงาน", data: dashboardCategoryValues, backgroundColor: "#6f9fce", borderRadius: 6 }] },
    ...
    window.addEventListener('load', () => {
        ...
        const rows = dashboardLatestTasks.map((task) => {
            ...
            return `<tr><td class="ps-4 fw-semibold">#T-${String(task.id).padStart(5, '0')}</td><td>${thaiDate(task.created_at)}</td><td><button class="btn btn-link p-0 text-start fw-semibold text-decoration-none dashboard-task-detail" data-task-id="${task.id}">${escapeHtml(task.title)}</button></td><td>${escapeHtml(task.department)}</td><td><span class="badge rounded-pill ${className}">${label}</span></td><td class="pe-4">${escapeHtml(task.created_by_name)}</td></tr>`;
        }).join('') || '<tr><td colspan="6" class="text-center text-muted py-4">ยังไม่มีข้อมูลงาน</td></tr>';
        card.innerHTML = `<div class="card-header d-flex align-items-center justify-content-between py-3 px-4"><div><h2 class="h5 mb-1 fw-bold page-heading">งานล่าสุด</h2><p class="text-muted small mb-0">ข้อมูลล่าสุดจากฐานข้อมูล</p></div><a class="btn btn-primary btn-sm px-3" href="../report/"><i class="bi bi-list-check me-1"></i>ดูทั้งหมด</a></div><div class="table-responsive"><table class="table table-hover mb-0"><thead><tr><th class="ps-4 py-3">รหัสงาน</th><th>วันที่สร้าง</th><th>หัวข้องาน</th><th>แผนก</th><th>สถานะ</th><th class="pe-4">ผู้สร้าง</th></tr></thead><tbody>${rows}</tbody></table></div>`;

        card.querySelectorAll('.dashboard-task-detail').forEach((button) => button.addEventListener('click', () => {
