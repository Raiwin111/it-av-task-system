...
if (!$dashboard_is_view_only && !$dashboard_can_filter_team) {
    ...
    // Tasks hidden by an admin never count toward team KPI cards.
    $dashboard_conditions[] = "is_visible = 1";
    $dashboard_conditions_t[] = "t.is_visible = 1";
} else if (!$dashboard_is_view_only && $dashboard_filter_team !== "") {
...
<main class="main-content flex-grow-1 p-4 p-lg-5">
    ...
    <section class="card table-card">
        <div class="card-header d-flex align-items-center justify-content-between py-3 px-4">
            <div><h2 class="h5 mb-1 fw-bold page-heading">งานล่าสุด</h2><p class="text-muted small mb-0">5 งานล่าสุดตามทีมและตัวกรองปัจจุบัน</p></div>
            <a class="btn btn-primary btn-sm px-3" href="<?php echo htmlspecialchars($dashboard_report_url, ENT_QUOTES, "UTF-8"); ?>"><i class="bi bi-list-check me-1"></i>รายการงานทั้งหมด</a>
        </div>
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                <thead><tr><th class="ps-4 py-3">ลำดับ</th><th class="optional-table-cell">วันที่สร้าง</th><th>ชื่องาน</th><th class="optional-table-cell">ทีม</th><th>สถานะ</th><th class="pe-4 optional-table-cell">ผู้รับผิดชอบ</th></tr></thead>
                <tbody>
                    <?php if (!$recent_task_rows): ?>
                        <tr><td colspan="6" class="text-center text-muted py-4">ยังไม่มีข้อมูลงาน</td></tr>
                    <?php else: ?>
                        <?php foreach ($recent_task_rows as $task_index => $task): ?>
                            <?php [$label, $class] = dashboard_status_meta($task["status"]); ?>
                            <?php $recent_displaySequence = (($recent_task_page - 1) * $recent_tasks_per_page) + $task_index + 1; ?>
                            <tr class="dashboard-task-row" data-task-id="<?php echo (int) $task["id"]; ?>" role="button" tabindex="0" aria-label="ดูรายละเอียดงาน <?php echo htmlspecialchars($task["title"], ENT_QUOTES, "UTF-8"); ?>">
                                <td class="ps-4 fw-semibold"><?php echo $recent_displaySequence; ?></td>
                                <td class="optional-table-cell"><?php echo thai_date_time($task["created_at"]); ?></td>
                                <td><button class="btn btn-link p-0 text-start fw-semibold text-decoration-none dashboard-task-detail" data-task-id="<?php echo (int) $task["id"]; ?>"><?php echo htmlspecialchars($task["title"], ENT_QUOTES, "UTF-8"); ?></button></td>
                                <td class="optional-table-cell"><span class="badge text-bg-light border"><?php echo htmlspecialchars($task["department"], ENT_QUOTES, "UTF-8"); ?></span></td>
                                <td><span class="badge rounded-pill <?php echo $class; ?>"><?php echo $label; ?></span></td>
                                <td class="pe-4 optional-table-cell"><?php echo htmlspecialchars($task["created_by_name"], ENT_QUOTES, "UTF-8"); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
        ...
        <?php if ($recent_task_total_pages > 1): ?>
            <?php $dashboard_page_start = max(1, $recent_task_page - 2); $dashboard_page_end = min($recent_task_total_pages, $recent_task_page + 2); ?>
            <div class="d-flex flex-column flex-sm-row align-items-center justify-content-between gap-2 px-4 py-3 border-top">
                <span class="small text-muted">แสดง <?php echo count($recent_task_rows); ?> จาก <?php echo $recent_task_total; ?> งาน</span>
                <nav aria-label="การแบ่งหน้างานล่าสุด"><ul class="pagination pagination-sm mb-0">
                    <li class="page-item<?php echo $recent_task_page <= 1 ? " disabled" : ""; ?>">
                        <?php if ($recent_task_page <= 1): ?><span class="page-link">ก่อนหน้า</span>
                        <?php else: ?><a class="page-link" href="?<?php echo htmlspecialchars(http_build_query(array_merge($dashboard_filter_query, ["recent_page" => $recent_task_page - 1])), ENT_QUOTES, "UTF-8"); ?>">ก่อนหน้า</a>
                        <?php endif; ?>
                    </li>
                    <?php if ($dashboard_page_start > 1): ?>
                        <li class="page-item"><a class="page-link" href="?<?php echo htmlspecialchars(http_build_query(array_merge($dashboard_filter_query, ["recent_page" => 1])), ENT_QUOTES, "UTF-8"); ?>">1</a></li>
                        <?php if ($dashboard_page_start > 2): ?><li class="page-item disabled"><span class="page-link">…</span></li><?php endif; ?>
                    <?php endif; ?>
                    <?php for ($dashboard_page_number = $dashboard_page_start; $dashboard_page_number <= $dashboard_page_end; $dashboard_page_number++): ?>
                        <li class="page-item<?php echo $dashboard_page_number === $recent_task_page ? " active" : ""; ?>"><a class="page-link" href="?<?php echo htmlspecialchars(http_build_query(array_merge($dashboard_filter_query, ["recent_page" => $dashboard_page_number])), ENT_QUOTES, "UTF-8"); ?>"><?php echo $dashboard_page_number; ?></a></li>
                    <?php endfor; ?>
                    <?php if ($dashboard_page_end < $recent_task_total_pages): ?>
                        <?php if ($dashboard_page_end < $recent_task_total_pages - 1): ?><li class="page-item disabled"><span class="page-link">…</span></li><?php endif; ?>
                        <li class="page-item"><a class="page-link" href="?<?php echo htmlspecialchars(http_build_query(array_merge($dashboard_filter_query, ["recent_page" => $recent_task_total_pages])), ENT_QUOTES, "UTF-8"); ?>"><?php echo $recent_task_total_pages; ?></a></li>
                    <?php endif; ?>
                    <li class="page-item<?php echo $recent_task_page >= $recent_task_total_pages ? " disabled" : ""; ?>">
                        <?php if ($recent_task_page >= $recent_task_total_pages): ?><span class="page-link">ถัดไป</span>
                        <?php else: ?><a class="page-link" href="?<?php echo htmlspecialchars(http_build_query(array_merge($dashboard_filter_query, ["recent_page" => $recent_task_page + 1])), ENT_QUOTES, "UTF-8"); ?>">ถัดไป</a>
                        <?php endif; ?>
                    </li>
                </ul></nav>
            </div>
