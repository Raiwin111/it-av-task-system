...
expect_true(
    task_activity_changed_labels(
        ["title" => "เดิม", "location" => "ห้องประชุม A"],
        ["title" => "ใหม่", "location" => "ห้องประชุม A"]
    ) === ["ชื่องาน"],
...
$task_ownership = $conn->query(
    "SELECT COUNT(*) AS total,
            SUM(created_by = (SELECT id FROM users WHERE username = 'admin_demo' LIMIT 1)) AS arm_total
     FROM tasks"
)->fetch_assoc();
expect_true(
...
expect_true(
    ...
    && str_contains($task_input_source, '"display_name" => "ระบบเสียงหลักขององค์กร"')
